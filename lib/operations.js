...
module.exports = {
    ...
    add: function(location, reviewers) {
        var srcFile = verifyAndGetScriptLocation(location);

        if (reviewers.length) {
            addReviewers(srcFile, reviewers);
        } else {
            promptReviewersForAddition(function(inputReviewers) {
                addReviewers(srcFile, inputReviewers);
            });
        }
    },

    remove: function(location, reviewers) {
        var srcFile = verifyAndGetScriptLocation(location);
        if (reviewers.length) {
            removeReviewers(srcFile, reviewers);
        } else {
            promptReviewersForRemoval(srcFile, function(inputReviewers) {
                removeReviewers(srcFile, inputReviewers);
            });
        }
    },
...
function verifyAndGetScriptLocation(location) {
    var srcFile = path.join(location, ".git", "hooks", "pre-push");
    if (!files.fileExists(srcFile)) {
...
function promptReviewersForAddition(done) {
    var questions = [{
        name: 'reviewer',
        type: 'input',
...
function promptReviewersForRemoval(filePath, done) {
    console.log(chalk.green("Loading existing reviewers, please wait..."));
    files.readLineContainsStr(filePath, 'reviewers=', function(line) {
        var currentReviewers = line.replace('reviewers="', '').replace('"', '').split(",");

        if (currentReviewers.length == 1 && currentReviewers[0] === '') {
            console.log(chalk.red("Oops.. We couldn't find any reviewer at this location"));
            terminate();
        }
        ...
        inquirer.prompt([{
            type: 'checkbox',
            name: 'selections',
            message: 'Please select reviewers from below.',
            choices: currentReviewers,
            validate: function(answer) {
                if (answer.length < 1) {
                    return 'You must choose at least one reviewer';
                }
                return true;
            }
        }]).then(function(answers) {
            done(answers['selections']);
        });
    });
}
...
function removeReviewers(filePath, reviewers) {
    var message = 'Removing reviewers to git-crucible-review-creator, please wait...';
    if (reviewers.length === 1) message = 'Removing a reviewer to git-crucible-review-creator, please wait...';
    var status = Spinner.create(message);
    status.start();
    files.readLineContainsStr(filePath, 'reviewers=', function(line) {
        var currentReviewers = line.replace('reviewers="', '').replace('"', '').split(",");
        var newReviewers = currentReviewers.filter(function(elem) {
            return reviewers.indexOf(elem) < 0 && elem !== '';;
        });

        var newLine = 'reviewers="' + newReviewers.join(',') + '"';
        files.replaceInFile(filePath, line, newLine);

        var updatedMessage = reviewers.join(', ') + ' are removed successfully as reviewers!';
        if (reviewers.length === 1) updatedMessage = reviewers[0] + ' is removed successfully as a reviewer!';
        Spinner.update(status, updatedMessage);
        status.stop();
    });
}
...
function handleLocationFound(location, action, done) {
    inquirer.prompt([{
        type: 'confirm',
        name: 'confirm',
        message: 'Are you sure you want to ' + action + ' the script from [' + location + '] ?'
    }]).then(function(answers) {
        if (answers['confirm']) {
            done(location);
        } else {
